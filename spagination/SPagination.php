...
class SPagination
{
	...
	private static function language($lang)
	{
		$lang = !empty(self::$lang) ? self::$lang : "en_EN";
		...
		{		
			...
			self::$firstAndLastLabel 	= [ucwords($data["FIRST_PAGE"]), ucwords($data["LAST_PAGE"])];
			self::$previousAndNextLabel = [ucwords($data["PREVIOUS_PAGE"]), ucwords($data["NEXT_PAGE"])];
		}
	}
}